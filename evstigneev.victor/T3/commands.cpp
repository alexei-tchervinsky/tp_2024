...
double countArea(evstigneev::Polygon& poly) {
  ...
  for (size_t i = 0; i < poly.points.size() - 1; ++i) {
    area += poly.points[i].x * poly.points[i + 1].y - 
      poly.points[i + 1].x * poly.points[i].y;
  }
  area += poly.points.back().x * poly.points.front().y - 
    poly.points.front().x * poly.points.back().y;
...
std::ostream& evstigneev::area(std::istream& in, std::ostream& out, 
...
double countAreasVexes(std::vector <evstigneev::Polygon>& poly, 
  int numOfVexes)
{
  return std::accumulate(poly.cbegin(), poly.cend(), 0, 
    std::bind(sumAreasIfEqual, std::placeholders::_1, std::placeholders::_2, 
      numOfVexes));
}
...
{
  return std::count_if(poly.begin(), poly.end(), std::bind(numOfVexesEqual, 
    std::placeholders::_1, numOfVexes));
}
