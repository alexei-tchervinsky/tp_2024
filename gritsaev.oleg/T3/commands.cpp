...
namespace gritsaev
{
    ...
    double get_area(std::size_t param, const std::vector< gritsaev::Polygon > polygons)
    {
    ...
    double get_max(std::size_t param, const std::vector< gritsaev::Polygon > polygons)
    {
        if (param == 0)
        {
    ...
    double get_min(std::size_t param, const std::vector< gritsaev::Polygon > polygons)
    {
        if (param == 0)
        {
    ...
    std::size_t get_count(std::size_t param, const std::vector< gritsaev::Polygon > polygons)
    {
        if (param == 1)
        {
    ...
    std::size_t get_seq(std::vector< gritsaev::Polygon >::const_iterator begin,
        std::vector< gritsaev::Polygon >::const_iterator end, const Polygon& param)
    {
        bool repeat = true;
        ...
        std::vector< gritsaev::Polygon >::const_iterator begin_new = std::find_if(begin, end, FindIfUO);
        if (begin_new != end)
        {
    ...
    void area_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        std::string param;
        in >> param;
        try
    ...
    void max_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        std::string param;
        in >> param;
    ...
    void min_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        std::string param;
        in >> param;
        if (param == "AREA" && polygons.size() >= 1)
    ...
    void count_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        std::string param;
        in >> param;
        try
    ...
    void rects_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        if (!in)
        {
            throw std::invalid_argument("<INVALID COMMAND>");
    ...
    void maxseq_param(const std::vector< gritsaev::Polygon > polygons, std::istream& in, std::ostream& out)
    {
        Polygon param;
        in >> param;
        if (!in || in.peek() != '\n')
