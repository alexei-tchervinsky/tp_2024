...
namespace nspace
{
        ...
        std::vector<Polygon>::const_iterator end, const Polygon& param)
    {
        ...
        std::vector<Polygon>::const_iterator begin_new = std::find_if(begin, end, FindIfUO);
        if (begin_new != end)
        {
    ...
    void intersections_param(const std::vector<Polygon>& polygons, std::istream& in, std::ostream& out)
    {
        ...
        std::size_t intersection_count = std::count_if(polygons.cbegin(), polygons.cend(),
            [&param](const Polygon& poly) { return intersects(param, poly); });
        out << std::setprecision(0) << intersection_count << '\n';
    }
}
