...
std::string mynspace::getText(const char* file)
{
...
std::string mynspace::toLower(std::string text)
{
  std::transform(
    text.begin(),
    text.end(),
...
std::string mynspace::cleanText(std::string text)
{
...
void mynspace::getTopOfNumber(dictionary& dict, std::ostream& out, int number)
{
  if (number == 0)
  {
    number = 3;
...
void mynspace::searchWord(dictionary& dict, std::ostream& out, std::istream& in)
{
  std::string word;
  in >> word;
  if (!in)
...
void mynspace::searchNumber(dictionary& dict, std::ostream& out, std::istream& in)
{
  std::size_t number;
  in >> number;
  if (!in)
...
void mynspace::deleteWord(dictionaries& dicts, std::ostream& out, std::istream& in)
{
  std::string fileName = "";
  in >> fileName;
...
void mynspace::deleteDictionary(dictionaries& dicts, std::ostream& out, std::istream& in)
{
  std::string fileName = "";
  in >> fileName;
  if (dicts.find(fileName) == dicts.end())
...
void mynspace::showAllDictionaries(dictionaries& dicts, std::ostream& out)
{
  if (dicts.empty())
  {
...
void mynspace::showAllWords(dictionary& dict, std::ostream& out)
{
  for (const auto& pair : dict)
  {
    out << pair.first << ": " << pair.second << '\n';
